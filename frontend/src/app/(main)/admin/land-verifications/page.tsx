...
export default function LandVerificationsPage() {
  ...
  const [stats, setStats] = useState<AdminStats>({
    total_lands: 0, pending_verification: 0, verified: 0, flagged: 0,
  });
  ...
  const handleVerify = async (id: number) => {
    ...
    try {
      ...
      setLands((prev) =>
        prev.map((l) =>
          l.id === id ? { ...l, is_verified: true, is_flagged: false, flag_reason: undefined } : l
        )
      );
      setStats((s) => ({
        ...s,
  ...
  const handleFlag = async (id: number, reason: string) => {
    ...
    try {
      ...
      setLands((prev) =>
        prev.map((l) =>
          l.id === id ? { ...l, is_verified: false, is_flagged: true, flag_reason: reason } : l
        )
      );
      setStats((s) => ({
        ...s,
  ...
  const getRowStatus = (l: LandListing) => {
    if (l.is_flagged)   return { label: "Flagged",  style: "bg-red-100 text-red-800 border-red-200",       dot: "bg-red-500" };
    if (l.is_verified)  return { label: "Verified", style: "bg-green-100 text-green-800 border-green-200", dot: "bg-green-500" };
    return                     { label: "Pending",  style: "bg-yellow-100 text-yellow-800 border-yellow-200", dot: "bg-yellow-500" };
  };
  ...
  return (
    <div className="p-5 lg:p-8">
      ...
      {toast && (
        <div
          className={`fixed top-5 right-5 z-50 flex items-center gap-3 rounded-xl px-5 py-3 shadow-lg text-sm font-semibold text-white transition-all ${
      ...
      {flagModalId !== null && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md mx-4">
            <h4 className="font-bold text-lg text-earth mb-2">Flag this Listing</h4>
            ...
            <div className="flex gap-3 justify-end">
              <button
                onClick={() => { setFlagModalId(null); setFlagReason(""); }}
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm max-w-xl">
            Manually cross-reference Title Deed Numbers with the national land registry
            to approve or flag listings.
          </p>
        </div>
        ...
        {statCards.map((card) => {
          ...
          return (
            <div key={card.label} className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between">
              <div>
                <h3 className="text-earth text-[10px] font-bold uppercase tracking-widest mb-1">{card.label}</h3>
              ...
              <span className={`w-12 h-12 rounded-full ${card.iconBg} flex items-center justify-center`}>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/30">
          <h3 className="text-base font-bold text-earth font-serif">Title Deed Verification Queue</h3>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  <th className="py-4 px-5 text-[10px] font-bold text-earth uppercase tracking-widest">Land / Owner</th>
                  <th className="py-4 px-5 text-[10px] font-bold text-earth uppercase tracking-widest">Plot ID</th>
                  ...
                  <th className="py-4 px-5 text-[10px] font-bold text-earth uppercase tracking-widest">Submitted</th>
                  <th className="py-4 px-5 text-[10px] font-bold text-earth uppercase tracking-widest">Status</th>
                  <th className="py-4 px-5 text-[10px] font-bold text-earth uppercase tracking-widest text-right">Action</th>
                </tr>
              </thead>
              <tbody className="text-sm divide-y divide-gray-50">
                {filtered.map((land) => {
                  ...
                  return (
                    <tr key={land.id} className="group hover:bg-gray-50/50 transition-colors">
                      {/* Land title + owner */}
                      <td className="py-4 px-5">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <span className="block font-bold text-gray-800 max-w-40 truncate">{land.title}</span>
                            <span className="text-xs text-gray-500">{land.total_area} Acres</span>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-5 bg-emerald-50/10 border-x border-gray-50 group-hover:bg-emerald-50/20">
                        <div
                          ...
                          onClick={() => copyDeed(land.id, land.title_deed_number ?? "")}
                          title="Click to copy"
                        >
                      ...
                      <td className="py-4 px-5">
                        <div className="flex flex-col gap-1">
                          <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold border ${rowStatus.style} w-fit`}>
                            <span className={`w-1.5 h-1.5 rounded-full ${rowStatus.dot}`} />
                            {rowStatus.label}
                          </span>
                          {land.is_flagged && land.flag_reason && (
                            <span className="text-[10px] text-red-500 max-w-40 truncate" title={land.flag_reason}>
                              {land.flag_reason}
                            </span>
                          )}
                        </div>
                      ...
                      <td className="py-4 px-5 text-right">
                        {!land.is_verified ? (
                          <div className="flex justify-end gap-2">
                            <button
                              onClick={() => { setFlagModalId(land.id); setFlagReason(""); }}
                              disabled={isActing}
        ...
        <div className="bg-gray-50 px-5 py-4 border-t border-gray-100 flex items-center justify-between">
          <p className="text-xs text-gray-500">
            ...
            <span className="font-bold text-gray-700">{lands.length}</span> listings
          </p>
          <div className="flex gap-2">
            <button disabled className="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-300 bg-white text-gray-300 cursor-not-allowed">
      ...
      <div className="mt-7 grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 bg-sidebar-bg rounded-xl p-6 text-white relative overflow-hidden shadow-lg">
          ...
          <div className="relative z-10">
            <h4 className="font-serif text-lg font-bold mb-2">Manual Verification Protocol</h4>
            <div className="flex items-center gap-2 mb-4">
              <span className="w-2 h-2 rounded-full bg-yellow-400" />
              <span className="text-xs font-medium text-yellow-200 uppercase tracking-wider">Review Required</span>
            </div>
            <ul className="text-sm text-green-100/80 mb-4 space-y-2 list-disc pl-4 max-w-lg">
              <li>Cross-check the <strong>Title Deed Number</strong> with the physical land registry or government portal.</li>
              <li>Verify the <strong>Owner</strong> matches the deed holder exactly.</li>
              <li>Confirm the <strong>Plot ID</strong> corresponds to the correct region.</li>
            </ul>
          </div>
        </div>

        <div className="bg-white rounded-xl p-6 border border-gray-100 shadow-sm">
          <h4 className="font-bold text-earth mb-4 text-sm uppercase tracking-widest">Quick Filters</h4>
          <div className="space-y-3">
            {(["pending", "verified", "flagged"] as const).map((f, i) => {
              const labels = ["Show Pending Only", "Verified Only", "Flagged for Correction"];
              return (
                <label key={f} className="flex items-center gap-2 cursor-pointer group">
                  ...
                  />
                  <span className="text-sm text-gray-600 group-hover:text-gray-900">{labels[i]}</span>
                </label>
              );
            })}
          </div>
