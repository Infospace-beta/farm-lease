  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden">
      <AdminPageHeader
        ...
        subtitle="Review and resolve lease disputes between tenants and landowners."
      />
      <div className="flex-1 overflow-y-auto p-4 lg:p-8 bg-[#f8fafc]">
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          {[
            ...
            <div
              key={s.label}
              className={`${s.bg} rounded-2xl p-5 border border-gray-100 shadow-sm text-center`}
            >
              ...
              <thead className="bg-gray-50 text-[10px] uppercase tracking-wider text-gray-500 font-bold border-b border-gray-100">
                <tr>
                  {[
                    ...
                    <th
                      key={h}
                      className={`px-5 py-3 ${h === "Action" ? "text-right" : ""}`}
                    >
                      {h}
                    </th>
                  ))}
                ...
                {disputes.map((d) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-4">
                      <span
                        className={`px-2.5 py-1 rounded-full text-xs font-bold ${d.priorityBg}`}
                      >
                        {d.priority}
                      </span>
                    </td>
                    <td className="px-5 py-4">
                      <span
                        className={`inline-flex items-center px-2.5 py-1 border rounded-full text-xs font-bold ${d.statusBg}`}
                      >
                        {d.status}
                      </span>
                    </td>
                    <td className="px-5 py-4 text-right">
                      <button
                        ...
                        className={`px-3 py-1.5 text-xs font-bold rounded-lg border transition ${d.status === "Resolved"
