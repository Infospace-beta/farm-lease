...
function getIcon(category: string): { icon: string; iconBg: string; iconColor: string } {
  if (category === "users") return { icon: "group", iconBg: "bg-blue-100", iconColor: "text-blue-600" };
  if (category === "verifications") return { icon: "verified", iconBg: "bg-emerald-100", iconColor: "text-[#047857]" };
  if (category === "leases") return { icon: "description", iconBg: "bg-purple-100", iconColor: "text-purple-600" };
  if (category === "payments") return { icon: "payments", iconBg: "bg-amber-100", iconColor: "text-amber-600" };
...
export default function AdminNotificationsPage() {
  ...
  const tabs = useMemo(() => {
    const counts = { all: notifications.length, users: 0, verifications: 0, leases: 0, payments: 0 };
    notifications.forEach((n) => {
      if (n.category === "users") counts.users++;
      else if (n.category === "verifications") counts.verifications++;
      else if (n.category === "leases") counts.leases++;
    ...
    return [
      ...
      { key: "verifications", label: "Verifications", count: counts.verifications },
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden">
      <AdminPageHeader
        title="Notifications"
        subtitle="System-wide alerts, user registrations and land verifications"
      >
        <button
