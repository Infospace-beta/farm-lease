                ...
                <div className="absolute z-20 w-full mt-2 bg-white border border-gray-200 rounded-xl shadow-xl max-h-80 overflow-y-auto">
                  {getSearchSuggestions().map((option, index) => (
                    <button
                      key={index}
                      onClick={() =>
                        handleSearchSelect(option.value, option.type)
                      }
        ...
        <div className="flex-1 overflow-hidden flex">
          ...
          <aside className="w-80 bg-white border-r border-gray-200 overflow-y-auto p-6 flex flex-col gap-8">
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                {selectedRegions.length === 0 ? (
                  <p className="text-xs text-gray-400 italic">
                    No regions selected yet. Start typing below to add regions.
                  </p>
                ) : (
                  selectedRegions.map((region) => (
                    <div
                      key={region}
                      className="inline-flex items-center bg-emerald-50 text-primary-dark px-3 py-1.5 rounded-full text-xs font-medium border border-emerald-200/50"
                    >
                      {region}
                      <button
                        onClick={() => removeRegion(region)}
                        className="ml-1.5 hover:text-emerald-800"
                      >
                        <X size={14} />
                      </button>
                    </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <div>
                  <label className="text-[10px] text-gray-400 block mb-2">
                    Min Acres: {minAcres}
                  </label>
                  <input
                    type="range"
          ...
          <div className="flex-1 bg-background-light p-8 overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {lands.map((land) => (
                <div
                  key={land.id}
                  className="bg-white rounded-2xl shadow-card overflow-hidden group hover:shadow-xl transition-all duration-300 border border-transparent hover:border-primary-dark/20 flex flex-col"
                >
                  <div className="relative h-48">
                    ...
                    <span
                      className={`absolute bottom-3 left-3 ${land.badgeColor} text-white px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide shadow-sm`}
                    >
                      {land.badge}
                    </span>
                  </div>
                  <div
                    className={`${land.id === 1 ? "pt-10" : "pt-4"} px-5 pb-5 flex flex-col flex-1`}
                  >
                    ...
                    <div className="grid grid-cols-3 gap-2 py-3 border-t border-b border-gray-100 mb-4">
                      <div className="text-center">
                        <div className="text-[10px] text-earth-brown uppercase tracking-wide">
                          Soil
                        </div>
                    ...
                    <div className="mt-auto flex items-center justify-between">
                      {land.pending ? (
                        ...
                        <div
                          className={`w-8 h-8 rounded-full ${land.matchColor} flex items-center justify-center`}
                        >
                      ...
                      <a
                        href="#"
                        className="text-xs font-bold text-primary-dark flex items-center hover:text-emerald-700 transition-colors"
                      >
                        View Details
