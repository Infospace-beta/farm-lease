...
function buildItem(req?: LeaseRequest, agr?: Agreement): LeaseItem {
  if (agr) {
    ...
    // Stage 3: submitted, owner not signed
    if (agr.lessee_submitted && !agr.owner_signed) {
      return {
        ...
        actionLabel: "Awaiting Owner", actionVariant: "muted", agreement: agr,
      };
    }
    ...
    if (agr.owner_signed && !agr.witness_signed_at && agr.status !== "active") {
      return {
        ...
        actionLabel: "Pending Witness", actionVariant: "muted", agreement: agr,
      };
    }
    // Stage 5: active
          ...
          <div key={label} className="flex-1 flex flex-col items-center gap-1">
            <div
              className={`h-1.5 w-full rounded-full transition-all ${
                done ? "bg-[#16a34a]" : active ? "bg-[#13ec80]" : "bg-slate-200"
              }`}
            />
            <span
              className={`text-[9px] font-semibold hidden sm:block ${
                done ? "text-emerald-600" : active ? "text-[#16a34a]" : "text-slate-300"
              }`}
            >
              {label}
            </span>
...
export default function LeasesPage() {
  ...
  const handleAction = (item: LeaseItem) => {
    if (item.agreement) {
      if (item.stage === 5) setPaymentAgreement(item.agreement);
                  ...
                  <div className="flex flex-col items-end gap-3 shrink-0">
                    ...
                    {item.actionLabel && (
                      item.actionVariant === "muted" ? (
                        ...
                        <button
                          onClick={() => handleAction(item)}
                          className={`text-xs font-bold px-5 py-2.5 rounded-xl text-white transition-colors flex items-center gap-2 ${
                            item.stage === 5 ? "bg-[#16a34a] hover:bg-emerald-700" : "bg-[#0f392b] hover:bg-emerald-900"
                          }`}
                        >
                    ...
                    {item.stage === 5 && (
                      <button
                        onClick={() => item.agreement && setActiveAgreement(item.agreement)}
